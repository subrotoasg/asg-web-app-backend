...
স্লাইড না। **১০টা পেজ, প্রতিটায় একটাই প্রশ্নের উত্তর: এইটা কেন করেছি?**

প্রতিটা পেজ দুই ভাগে —
বাঁয়ে **"যদি না করতাম"** (কী ভাঙত, চলন্ত ছবিতে),
ডানে **"তাই যা করেছি"** (কীভাবে ঠিক হলো)।
দুইটা এনিমেশনই নিজে থেকে ধীরে ধীরে চলতে থাকে, বন্ধ করতে হয় না।
**পেজ বদলানো পুরোপুরি তোমার হাতে** — যতক্ষণ খুশি এক পেজে থাকো।

| পেজ | কেন করেছি |
|---|---|
| ০১ · কিউ | API নিজে লুপ চালালে ২৭ ঘণ্টা + টাইমআউট → জব Redis-এ, ৪০ms-এ 200 OK |
| ০২ · ক্যাশ | প্রতি রিকোয়েস্টে ৩–৪ DB কোয়েরি → অথ চেক শূন্য কোয়েরিতে |
| ০৩ · singleFlight + লক | ক্যাশ মিসের মুহূর্তে সবাই একসাথে DB-তে → একজন যায়, বাকিরা অপেক্ষা |
| ০৪ · Lua লক | সাধারণ `DEL` অন্যের লক মুছে দেয় → টোকেন মিলিয়ে তবেই DEL |
| ০৫ · ভার্সন | `SCAN` চালালে Redis আটকে যায় → একটা `INCR` |
| ০৬ · কার্সর | `skip` এ প্রতি পেজ ধীর হতে থাকে → cursor-এ সব পেজ সমান দ্রুত |
| ০৭ · ব্যাচ + থ্রটল | একসাথে ছাড়লে FCM ৪২৯ + নিজের DB বসে → ৫০০ করে, সেকেন্ডে ১০ ব্যাচ |
| ০৮ · রিট্রাই | এক এরর = ইউজার বাদ → ৩ বার, প্রতিবার দ্বিগুণ অপেক্ষা |
| ০৯ · স্ট্রিম | Pub/Sub-এ ওয়ার্কার ক্র্যাশ = মেসেজ শেষ → ACK না হলে থেকে যায় |
| ১০ · fallback | Redis ধীর হলে সব ঝুলে যায় → ১৫০০ms এ হাল ছেড়ে সোজা DB |

কন্ট্রোল: `←` `→` পেজ · `F` ফুলস্ক্রিন · নিচের বার থেকে যেকোনো পেজে ক্লিক।

রেকর্ড করার সময় এক পেজে থেমে দুই পাশের এনিমেশন দেখিয়ে বলো — বাঁয়েরটা
সমস্যা, ডানেরটা সমাধান। পেজের নিচের বুলেটগুলোই তোমার কথার কাঠামো।
