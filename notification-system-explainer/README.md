...
স্লাইড না, **চলমান মেকানিজম**। আটটা দৃশ্য, ৫৯টা ধাপ — প্রতিটা ধাপে রিকোয়েস্ট,
জব আর ডেটা আসলে কোথায় যাচ্ছে সেটা নড়ে-চড়ে দেখা যায়, নিচে বাংলা ন্যারেশন সহ।

| দৃশ্য | কী দেখায় |
|---|---|
| ১ · হিট | ক্যাশ হিট — DB ছোঁয়াই লাগে না |
| ২ · মিস + লক | ৬টা রিকোয়েস্ট → singleFlight → Redis লক → পোলিং → **১টা DB কোয়েরি** |
| ৩ · ভার্সন | `INCR` দিয়ে ইনভ্যালিডেশন, `SCAN` ছাড়াই |
| ৪ · জব | API জব ঢোকায়, ৪০ms-এ 200 OK — নিজে কিছু পাঠায় না |
| ৫ · ওয়ার্কার | লক → কার্সর পেজ → ৫০০-এর ব্যাচ → থ্রটল → FCM → কাউন্টার |
| ৬ · রিট্রাই | ফেল → backoff ১০s/২০s/৪০s → failed set |
| ৭ · স্ট্রিম | `XADD` → `XACK` → ওয়ার্কার ক্র্যাশ → `XAUTOCLAIM` |
| ৮ · ডাউন | Redis মরে গেলে `catch` ধরে সরাসরি DB |

স্ক্রিনে **DB কোয়েরি** আর **Redis কমান্ড** কাউন্টার চলতে থাকে — পার্থক্যটা
সংখ্যায় দেখা যায়।

কন্ট্রোল: `Space` চালাও/থামাও · `←` `→` এক ধাপ · `১`–`৮` দৃশ্য · `F` ফুলস্ক্রিন ·
নিচের বার থেকে গতি (০.৬×–২×), আর যেকোনো ধাপে সরাসরি ক্লিক।

রেকর্ড করার সময় **অটোপ্লে চালিয়ে দিয়ে বলতে থাকো** — প্রতিটা ধাপের ক্যাপশনই
তোমার স্ক্রিপ্ট, ওটা পড়লেই হবে। ব্যাখ্যা লম্বা হলে গতি ০.৬× করে নাও।
